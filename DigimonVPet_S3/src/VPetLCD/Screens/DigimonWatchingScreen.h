...
namespace V20{
  ...
  class DigimonWatchingScreen : public VPetLCD::Screen{
    ...
    public:
      ...
      void setNumberOfPoop(uint8_t _numberOfPoop){
        // Safety clamp for ESP32-S3 port: prevents corrupted/uninitialized save/RAM
        // values from filling the entire LCD with poop sprites.
        numberOfPoop = min(_numberOfPoop, (uint8_t)4);
        if(numberOfPoop == 0 && !isFlushing){
